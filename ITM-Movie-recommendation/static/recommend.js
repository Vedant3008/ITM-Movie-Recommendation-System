$(function() {
  ...
  $('.movie-button').on('click',function(){
    var my_api_key = 'e0a0015ede8a824fab642d5d1660255e';
    var title = $('.movie').val();
    if (title=="") {
...
//This snippet copied as we were getiing an error as we were not able to preprocess the data properly as it was 
//getting harder for me to manage the api dataset and the local dataset
...
function show_details(movie_details,movie_title,my_api_key,movie_id,movie_title_org){
  ...
  //Why using JSON.stringify() so that we can convert the JS into string so that we can 
  //post object to show in the main.py file
  details = {
      'title':movie_title,
...
function get_movie_cast(movie_id,my_api_key){
  ...
  //copied the ajax call for recommendation from themoviedb.org docs
